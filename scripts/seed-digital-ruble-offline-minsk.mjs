...
const STEP_TITLES = {
  1: 'QR-инвойс',
  2: 'Подтверждение в приложении',
  3: 'Постановка в офлайн-очередь',
  4: 'Передача по mesh',
  5: 'Приём и дедупликация на gateway',
  6: 'Доставка на edge node',
  7: 'Отправка в Core',
  8: 'Финализация и обновление балансов',
  9: 'Возврат подтверждения',
};
...
const NODES = [
  ...
  { tempId: 'mesh-1', type: 'MESH_RELAY', role: 'mesh', name: 'Mesh-1', lat: 53.9031, lng: 27.5627, step: 4 },
  { tempId: 'mesh-2', type: 'MESH_RELAY', role: 'mesh', name: 'Mesh-2', lat: 53.9035, lng: 27.5636, step: 4 },
  { tempId: 'mesh-3', type: 'MESH_RELAY', role: 'mesh', name: 'Mesh-3', lat: 53.9039, lng: 27.5644, step: 4 },
  { tempId: 'mesh-4', type: 'MESH_RELAY', role: 'mesh', name: 'Mesh-4', lat: 53.9043, lng: 27.5652, step: 4 },
  { tempId: 'mesh-5', type: 'MESH_RELAY', role: 'mesh', name: 'Mesh-5', lat: 53.9047, lng: 27.566, step: 4 },
  { tempId: 'mesh-6', type: 'MESH_RELAY', role: 'mesh', name: 'Mesh-6', lat: 53.905, lng: 27.5665, step: 4 },
  {
...
const LINKS = [
  ...
  // B) Primary uplink
  { from: 'mesh-6', to: 'gateway-primary', kind: 'primary', step: 5, type: 'CABLE_UNDERGROUND_FIBER', name: 'Primary uplink: Mesh-6 → Gateway-Primary' },
  { from: 'gateway-primary', to: 'edge-a', kind: 'primary', step: 6, type: 'CABLE_UNDERGROUND_FIBER', name: 'Primary uplink: Gateway-Primary → Edge Node A' },
  { from: 'edge-a', to: 'core', kind: 'primary', step: 7, type: 'CABLE_UNDERGROUND_FIBER', name: 'Primary uplink: Edge Node A → Core' },

  // C) Backup uplink
  { from: 'mesh-3', to: 'gateway-backup', kind: 'backup', step: 5, type: 'CABLE_UNDERGROUND_COPPER', name: 'Backup uplink: Mesh-3 → Gateway-Backup' },
  { from: 'gateway-backup', to: 'edge-b', kind: 'backup', step: 6, type: 'CABLE_UNDERGROUND_COPPER', name: 'Backup uplink: Gateway-Backup → Edge Node B' },
  { from: 'edge-b', to: 'core', kind: 'backup', step: 7, type: 'CABLE_UNDERGROUND_COPPER', name: 'Backup uplink: Edge Node B → Core' },
...
function nodePayload(node) {
  return {
    ...
    metadata: {
      ...
      role: node.role,
      step: node.step ?? null,
      stepTitle: node.step ? STEP_TITLES[node.step] ?? null : null,
...
function linkPayload(link, fromNode, toNode) {
  ...
  return {
    ...
    metadata: {
      ...
      linkKind: link.kind,
      step: link.step ?? null,
      stepTitle: link.step ? STEP_TITLES[link.step] ?? null : null,
